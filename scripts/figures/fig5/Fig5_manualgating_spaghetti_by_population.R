...
# trajectories (light background lines) with SynF/CtrlF group medians overlaid
# as bold colored lines. Uses cytof_manual_clean (outlier excluded, plate
...
p <- ggplot2::ggplot(long, ggplot2::aes(x = age_days, y = value)) +
  ggplot2::geom_line(ggplot2::aes(group = subject_id), color = "grey75", linewidth = 0.25, alpha = 0.5) +
  ggplot2::geom_line(
    data = medians,
