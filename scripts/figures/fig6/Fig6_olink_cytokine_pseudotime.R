# Immune maturation pseudotime from Olink NPX data, ALTERNATIVE built only
# from age-associated CYTOKINES (OLINK_CYTOKINE_PROTEINS, common.R), rather
# than the top age-associated proteins of any type used in
# Fig6_olink_pseudotime.R -- per Petter's request, 2026-09-06, mirroring the
# same cytokines-only restriction already applied to the biplot
# (Fig5_olink_cytokine_only_biplot.R vs Fig5_olink_top_age_biplot.R).
#
# Protein selection: top 20% of the 59 classified cytokines by eta-squared
# vs timepoint (reused from output/tables/olink_age_association.csv,
# filtered to the cytokine list), same selection fraction as the
# any-protein-type pseudotime and the CyTOF pseudotime script, for
# consistency -- just applied within a smaller, cytokine-only candidate pool.
#
# Method otherwise identical to Fig6_olink_pseudotime.R (redundancy filter,
# PCA + principal curve, orientation/z-scoring, and the same three
# SynF-vs-CtrlF tests) -- see that script's header for full rationale.
#
# Output:
...
mat$SampleID <- NULL
cytokine_cols <- intersect(colnames(mat), OLINK_CYTOKINE_PROTEINS)
...
# ---- 1. Top 20% age-associated CYTOKINES (eta^2, reused from Fig5_olink_pca_age.R) ----
age_association <- readr::read_csv(file.path(root, "output", "tables", "olink_age_association.csv"), show_col_types = FALSE) |>
  dplyr::filter(protein %in% cytokine_cols)
n_top <- max(2, round(0.20 * length(cytokine_cols)))
...
cat("Top", n_top, "age-varying cytokines (of", length(cytokine_cols), "classified cytokines; eta^2 vs timepoint, redundancy-filtered):\n")
print(age_association |> dplyr::filter(protein %in% top_proteins) |> dplyr::arrange(dplyr::desc(eta_squared)))

# ---- 2. PCA on the top time-varying cytokines, all samples pooled ----
...
cat("\nSpearman correlation, cytokine pseudotime vs. nominal age (sanity check, should be strongly positive): ",
    round(age_pseudotime_cor, 3), "\n", sep = "")

readr::write_csv(pt_df, file.path(root, "output", "tables", "olink_cytokine_pseudotime_values.csv"))
...
readr::write_csv(pseudotime_change, file.path(root, "output", "tables", "olink_cytokine_pseudotime_change_from_baseline.csv"))
readr::write_csv(
  dplyr::bind_rows(
  ...
  file.path(root, "output", "tables", "olink_cytokine_pseudotime_group_tests.csv")
)
...
p1 <- ggplot2::ggplot(pca_plot_df, ggplot2::aes(x = PC1, y = PC2, color = pseudotime)) +
  ...
  ggplot2::labs(title = "Olink cytokine pseudotime: colored by pseudotime") +
...
p3 <- ggplot2::ggplot() +
  ...
  ggplot2::labs(x = "Immune maturation age (cytokine pseudotime, z-scored)", y = NULL) +
  ggplot2::theme_bw(base_size = 9) +
